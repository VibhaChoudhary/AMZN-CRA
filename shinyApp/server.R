...
function(input, output) {
  ...
  total_counts <- reactive({
    dir1 = paste0("D://Data//output//RData//total")
  ...
  category_counts <- reactive({
   if(input$category!="All"){
    dir1 = paste0("D://Data//output//RData//")
  ...
  output$header <- renderText({ 
     if(input$category!="All")
     {
       print(paste0("Showing analysis for category ",input$category))
     }
     else{
       total_counts()
       print(paste0("Showing analysis for all ",fcount," categories"))
     }
  })
  ...
  output$stats <- renderTable({
   ...
   output$ratings <- renderTable({
    ...
    if(input$category!="All")
    {
     ...
     records = data.table("Star ratings" =c("5 stars","4 stars","3 stars","2 stars","1 star"),"Count" = c(count_5,count_4,count_3,count_2,count_1))
     head(t(records))
    }
     else{
       total_analysis()
   ...
   output$year <- renderDataTable({
     ...
     if(input$category!="All")
     {
       ...
       DT::datatable(t(year_reviews) ,colnames=NULL,rownames=NULL,options = list(scrollX = TRUE,dom = 'tip'))
       #head((year_reviews),n=20)
     }
     else{
       total_analysis()
       DT::datatable(t(all_y) ,colnames=NULL,rownames=NULL,options = list(scrollX = TRUE,dom = 'tip'))
       #head((all_y),n=20)
     }
   }
   #,bordered = TRUE,spacing = 'xs',colnames=FALSE
